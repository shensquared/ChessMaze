...
public class SudokuDatabase {
	public static final String DATABASE_NAME = "ChessMaze";


	public static final String SUDOKU_TABLE_NAME = "chessmaze";
	 ...
	 * @return
	 */
	public DroidChessController getSudoku(DroidChessController s, long sudokuID) {
		SQLiteQueryBuilder qb = new SQLiteQueryBuilder();
	...
	public FolderInfo getFolderInfoFull(long folderID) {
		...
		try {
			...
			while (c.moveToNext()) {
				...
				if (state == DroidChessController.GAME_STATE_COMPLETED) {
					folder.solvedCount += count;
				}
				if (state == DroidChessController.GAME_STATE_PLAYING) {
	...
	public Cursor getSudokuList(long folderID, SudokuListFilter filter) {
		...
		if (filter != null) {
			if (!filter.showStateCompleted) {
				qb.appendWhere(" and " + SudokuColumns.STATE + "!=" + DroidChessController.GAME_STATE_COMPLETED);
			}
			if (!filter.showStateNotStarted) {
				qb.appendWhere(" and " + SudokuColumns.STATE + "!=" + DroidChessController.GAME_STATE_NOT_STARTED);
			}
			if (!filter.showStatePlaying) {
				qb.appendWhere(" and " + SudokuColumns.STATE + "!=" + DroidChessController.GAME_STATE_PLAYING);
			}
		}
	...
	/*
	public long insertSudoku(long folderID, DroidChessController sudoku) {
		SQLiteDatabase db = mOpenHelper.getWritableDatabase();
		ContentValues values = new ContentValues();
...
/*		if (!CellCollection.isValid(pars.data, CellCollection.DATA_VERSION_PLAIN)) {
	 ...
	 * @param sudoku
	 */
	public void updateSudoku(DroidChessController sudoku) {
		ContentValues values = new ContentValues();
		byte[] data = sudoku.toByteArray();
		String dataStr = byteArrToString(data);
		// System.out.println("this is the datastr" + dataStr);
		values.put(SudokuColumns.PUZZLE_NOTE, dataStr);
		values.put(SudokuColumns.LAST_PLAYED, sudoku.getLastPlayed());
		values.put(SudokuColumns.STATE, sudoku.getState());
		// values.put(SudokuColumns.GAME_VERSION, 3);

		SQLiteDatabase db = mOpenHelper.getWritableDatabase();
		db.update(SUDOKU_TABLE_NAME, values, SudokuColumns._ID + "=" + sudoku.getId(), null);
	}

	public void updateStartFen(String fen, int id) {
		ContentValues values = new ContentValues();
		values.put(SudokuColumns.DATA, fen);
		// values.put(SudokuColumns.EDITSTATE, editState);

		SQLiteDatabase db = mOpenHelper.getWritableDatabase();
		db.update(SUDOKU_TABLE_NAME, values, SudokuColumns._ID + "=" + id, null);
	}
	/**
