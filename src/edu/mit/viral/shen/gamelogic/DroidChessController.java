...
import edu.mit.viral.shen.EngineOptions;
import edu.mit.viral.shen.GUIInterface;
import edu.mit.viral.shen.GameMode;
import edu.mit.viral.shen.PGNOptions;
import edu.mit.viral.shen.Util;
import edu.mit.viral.shen.book.BookOptions;
import edu.mit.viral.shen.engine.DroidComputerPlayer;
import edu.mit.viral.shen.engine.DroidComputerPlayer.SearchRequest;
import edu.mit.viral.shen.engine.DroidComputerPlayer.SearchType;
import edu.mit.viral.shen.gamelogic.Game.GameState;
...
public class DroidChessController {
    ...
    public static final int EDIT_STATE_COMPLETED = 3;



    private long mId;
    private long mCreated;
    private int mState = GAME_STATE_NOT_STARTED;
    private int mGame_version;
    private long mLastPlayed;
    private String mData;
    private String mNote = "hello";
    ...
    public void saveState(Bundle outState) {
        ...
        outState.putInt("game_version", 3);
    }
    ...
    public void restoreState(Bundle inState) {
        ...
        mGame_version=inState.getInt("game_version");
    }
    ...
    public final synchronized void newGame(GameMode gameMode, TimeControlData tcData) {
        ...
        searchId++;
        String startPosition = getData();
        game = new Game(gameTextListener, tcData, startPosition);
...
/*
    /** Start a new game. 
    public final synchronized void newGame(GameMode gameMode, TimeControlData tcData, long game_id) {
        boolean updateGui = abortSearch();
        if (updateGui)
            updateGUI();
        this.gameMode = gameMode;
        ...
        computerPlayer.queueStartEngine(searchId, engine);
        searchId++;
        game = new Game(gameTextListener, tcData, game_id);
        computerPlayer.clearTT();
        setPlayerNames(game);
        updateGameMode();
    }
...
*/
 ...
 public final synchronized void setFENOrPGN(String fenPgn) throws ChessParseError {
        Game newGame = new Game(gameTextListener, game.timeController.tcData, "8/8/8/8/8/8/8/8 w KQkq - 1 1");
        try {
    ...
    public final synchronized void reportPromotePiece(int choice) {
        ...
        switch (choice) {
            case 0:
                promoteTo = white ? Piece.WPAWN : Piece.BPAWN;
                break;
            case 1:
    ...
    private final class SearchListener implements edu.mit.viral.shen.gamelogic.SearchListener {
