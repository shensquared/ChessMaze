...
import edu.mit.viral.shen.gamelogic.TimeControlData.TimeControlField;


public class GameTree {

    static public final List<String> repo =  Arrays.asList("8/8/8/8/8/8/8/8 w KQkq - 1 1",
    "DK6/8/1p3r2/8/8/2b2b2/8/8 w KQkq - 1 1",
    ...
    public GameTree(PgnToken.PgnTokenReceiver gameStateListener, String startPosition) {
        this.gameStateListener = gameStateListener;
        try {
            setStartPos(copyReadFEN(startPosition));
            } catch (ChessParseError e) {
        }
    } 

  /*  public GameTree(PgnToken.PgnTokenReceiver gameStateListener, long game_id) {
        this.gameStateListener = gameStateListener;
        try {
            setStartPos(copyReadFEN(startFEN(game_id)));
            } catch (ChessParseError e) {
        }
    } 
*/

    public final String startFEN(long game_id){
        int id = safeLongToInt(game_id);
        return repo.get(id);
    }
    ...
    final Position copyReadFEN(String fen) throws ChessParseError {
        ...
        for (int i = 0; i < words[0].length(); i++) {
            ...
            switch (c) {
                ...
                case 'D': safeSetPiece(pos, col, row, Piece.DEST); col++;break;//aug8
    ...
    final GameState getGameState() {
       ...
       // ArrayList<Move> moves = new MoveGen().legalMoves(pos);
        ...
        else if (pos.getKingSq(pos.whiteMove)==pos.getDestSq())//aug8
            return GameState.WHITE_MATE;
             /*else {
